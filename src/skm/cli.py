...
from skm.agents import DEFAULT_AGENT_SPECS, resolve_agent_specs
...
def _resolve_list_agents(config_path: Path, agents_dir: str | None = None):
    ...
    try:
        ...
        return resolve_agent_specs(None, agents_dir=agents_dir)
    return resolve_agent_specs(config.agents, agents_dir=agents_dir, selected_names=list(DEFAULT_AGENT_SPECS))
...
def install(ctx, source, skill_name, force, verbose, agents_includes, agents_excludes):
    ...
    if source is None:
        ...
        agents = resolve_agent_specs(config.agents, agents_dir=ctx.obj['agents_dir'])
        run_install(
            config=config,
    ...
    default_agents = resolve_agent_specs(config.agents, agents_dir=ctx.obj['agents_dir'])
